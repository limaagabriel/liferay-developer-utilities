...
lp() {
    ...
    case "$namespace/$command" in
        worktree/cd|bundle/cd|worktree/set|worktree/unset|worktree/get|worktree/root|portal/cdm|portal/gw|portal/sf|worktree/add|modules/changed|modules/deploy)
            local _cd_args=("${@:3}")
            set -- "${_cd_args[@]}"
            source "$script"
            ;;
